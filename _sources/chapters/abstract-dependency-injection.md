...
It is time.
We are ready for abstraction depdendency injection.

```{hint}
In abstract dependency injection we inject the composed composed dependency as opposed to construct it, and the dependency is abstract as opposed to concrete.
```
...
{
  protected ICharCipher charCipher;
